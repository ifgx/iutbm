...
comuted_len = computed_len + matrix[1][tmp]
	...
	\subsection{Couplage}
	    ...
	    Nous chercherons ensuite le flot maximale qui parcourt ce graphe. Pour trouver le flot maximale qui parcoure un graphe on utilisera l'algorithme 
	    de flot maximum de Ford-Fulkerson.

        \paragraph{Définition d’un flot sur un graphe}

	    Dans la théorie des graphes, un réseau à flot est un graphe directe ou chaque arc à une capacité et un flot, il va d'un sommet initiale dit 
	    source à un sommet sortie dit puit. 
...
\end{figure}
      ...
      La source est liée à tout les clients par un arc de capacité égale à 1 étant donné qu'un client ne peut pas choisir plusieurs pizzas.
      Ensuite chaque pizza est reliée au puit par un arc de capacité 1, chaque pizza ne peut être sélectionnée qu'une seule fois.
...
Ici on a trois clients et trois pizzas disponibles. Le client 1 n'aime que la pizza 1, le client 2 aime la pizza 1 et 3, quand au client 3 il aime la pizza 1 et 2.
