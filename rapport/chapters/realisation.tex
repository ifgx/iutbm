...
comuted_len = computed_len + matrix[1][tmp]
	...
	\subsection{Couplage}
        ...
        \paragraph{Définition d’un flot sur un graphe}
            ...
            A chaque sommet x, on associe deux ensembles d'arcs :
            \begin{itemize}
                \item $ \omega^+ (x)= $ ensemble des arcs d'origine x et d'extrémité différente de x

                \item $ \omega^- (x)= $ ensemble des arcs d'extrémité x et d'origine différente de x
